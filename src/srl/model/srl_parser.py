...
class SrlParser(pl.LightningModule):
    def __init__(
        ...
        number_dependency_labels = self._get_num_dependency_labels(
        ...
        self.argument_sequence_encoder = SequenceEncoder(
            ...
            input_size=predicate_timestep_encoding_size
            + argument_timestep_encoding_size,
    ...
    def forward(self, x):
        ...
        if self.use_dependency_trees:
            ...
            node_embeddings = torch.zeros(
                ...
                word_embeddings.size(1),
                flattened_node_embeddings.size(-1),
                device=flattened_node_embeddings.device,
                dtype=flattened_node_embeddings.dtype,
            )
            length_until_now = 0
            for i, sentence_length in enumerate(sentence_lengths):
                new_sent_end = length_until_now + sentence_length
                node_embeddings[i, 0:sentence_length] = flattened_node_embeddings[
        ...
        predicate_timestep_encodings = predicate_timestep_encodings.unsqueeze(2).expand(
            -1, -1, timesteps, -1
        )

        argument_timestep_encodings = self.argument_timestep_encoder(sequence_states)
        argument_timestep_encodings = argument_timestep_encodings.unsqueeze(1).expand(
            -1, timesteps, -1, -1
        )

        predicate_argument_states = torch.cat(
            (predicate_timestep_encodings, argument_timestep_encodings), dim=-1
        )
        ...
        argument_sequence_lengths = torch.repeat_interleave(
            sentence_lengths, num_predicates
        )
    ...
    def training_step(self, batch, batch_index):
        ...
        self.log(
            "train/loss/predicate_identification",
            step_result["predicate_identification_loss"],
        )
        self.log(
            "train/loss/sense_classification", step_result["sense_classification_loss"]
        )
        self.log(
            "train/loss/argument_classification",
            step_result["argument_classification_loss"],
        )
    ...
    def _shared_step(
        ...
        if compute_loss:
            ...
            if torch.isnan(loss) or not torch.isfinite(loss):
                ...
                self.print(
                    "Predicate identification loss:", predicate_identification_loss
                )
                self.print("Predicate disambiguation loss:", sense_classification_loss)
                self.print(
                    "Argument classification loss:", argument_classification_loss
                )
        ...
        if use_sense_candidates:
            ...
            sense_mask = torch.full(scores["senses"].shape, float("-inf")).to(
                self.device
            )
        ...
        if compute_predictions:
            ...
            step_output["predictions"] = self.get_senses_and_roles(
                predicate_indices,
                scores,
                sample["sentence_ids"],
                sample["sentence_lengths"],
            )
    ...
    def _shared_epoch_end(self, outputs, stage, compute_metrics=False):
        if compute_metrics:
            ...
            predicate_identification_loss = torch.stack(
                [x["predicate_identification_loss"] for x in outputs]
            ).mean()
            sense_classification_loss = torch.stack(
                [x["sense_classification_loss"] for x in outputs]
            ).mean()
            argument_classification_loss = torch.stack(
                [x["argument_classification_loss"] for x in outputs]
            ).mean()
        ...
        if predictions:
            if self.predictions_path is None:
                self.predictions_path = os.path.join(
                    self.trainer.logger.save_dir, f"{stage}_predictions.json"
                )
    ...
    def compute_step_metrics(self, scores, labels):
        ...
        predicate_tp = (
            predicates_p[torch.logical_and(predicates_g >= 0, predicates_p == 1)]
            == predicates_g[torch.logical_and(predicates_g >= 0, predicates_p == 1)]
        ).sum()
        predicate_fp = (
            predicates_p[torch.logical_and(predicates_g >= 0, predicates_p == 1)]
            != predicates_g[torch.logical_and(predicates_g >= 0, predicates_p == 1)]
        ...
        predicate_fn = (
            predicates_p[predicates_g == 1] != predicates_g[predicates_g == 1]
        ).sum()
        ...
        role_tp = (
            roles_p[torch.logical_and(roles_g >= 0, roles_p >= 1)]
            == roles_g[torch.logical_and(roles_g >= 0, roles_p >= 1)]
        ).sum()
        role_fp = (
            roles_p[torch.logical_and(roles_g >= 0, roles_p >= 1)]
            != roles_g[torch.logical_and(roles_g >= 0, roles_p >= 1)]
    ...
    def get_senses_and_roles(
        ...
        for (sentence_index, predicate_index), sense_id, predicate_role_scores in zip(
            predicate_indices, sense_ids, role_scores
        ):
            ...
            if not self.use_viterbi_decoding:
                predicate_role_ids = torch.argmax(
                    predicate_role_scores, dim=-1
                ).tolist()
            else:
            ...
            output[sentence_id][predicate_index] = {
                ...
                "roles": [
                    self.id2role[role_id]
                    for role_id in predicate_role_ids[:sentence_length]
                ],
            }
    ...
    def _get_num_dependency_labels(self, dependency_labels_vocab_path: Optional[str]):
        if dependency_labels_vocab_path is None:
            return 0
        else:
            with open(dependency_labels_vocab_path, "r") as f:
                dependency_labels = json.load(f)

            return len(dependency_labels)
    ...
    def _build_transition_matrices(self):
        ...
        self.role_transition_matrix = nn.Parameter(
            torch.as_tensor(_role_transition_matrix), requires_grad=False
        )
        self.role_start_transitions = nn.Parameter(
            torch.as_tensor(_role_start_transitions), requires_grad=False
        )
    ...
    def _compute_epoch_metrics(outputs):
        predicate_tp = torch.stack(
            [o["metrics"]["predicate_tp"] for o in outputs]
        ).sum()
        predicate_fp = torch.stack(
            [o["metrics"]["predicate_fp"] for o in outputs]
        ).sum()
        predicate_fn = torch.stack(
            [o["metrics"]["predicate_fn"] for o in outputs]
        ).sum()

        predicate_precision = (
            torch.true_divide(predicate_tp, (predicate_tp + predicate_fp))
            if predicate_tp + predicate_fp > 0
            else torch.as_tensor(0)
        )
        predicate_recall = (
            torch.true_divide(predicate_tp, (predicate_tp + predicate_fn))
            if predicate_tp + predicate_fn > 0
            else torch.as_tensor(0)
        )
        predicate_f1 = (
            2
            * torch.true_divide(
                predicate_precision * predicate_recall,
                predicate_precision + predicate_recall,
            )
            if predicate_precision + predicate_recall > 0
            else torch.as_tensor(0.0)
        )

        sense_correct = torch.stack(
            [o["metrics"]["sense_correct"] for o in outputs]
        ).sum()
        sense_total = torch.stack([o["metrics"]["sense_total"] for o in outputs]).sum()
        sense_accuracy = (
            torch.true_divide(sense_correct, sense_total)
            if sense_total > 0
            else torch.as_tensor(0.0)
        )
        ...
        role_precision = (
            torch.true_divide(role_tp, (role_tp + role_fp))
            if role_tp + role_fp > 0
            else torch.as_tensor(0)
        )
        role_recall = (
            torch.true_divide(role_tp, (role_tp + role_fn))
            if role_tp + role_fn > 0
            else torch.as_tensor(0)
        )
        role_f1 = (
            2
            * torch.true_divide(
                role_precision * role_recall, role_precision + role_recall
            )
            if role_precision + role_recall > 0
            else torch.as_tensor(0.0)
        )
        ...
        overall_precision = (
            torch.true_divide(overall_tp, (overall_tp + overall_fp))
            if overall_tp + overall_fp > 0
            else torch.as_tensor(0.0)
        )
        overall_recall = (
            torch.true_divide(overall_tp, (overall_tp + overall_fn))
        ...
        overall_f1 = (
            2
            * torch.true_divide(
                overall_precision * overall_recall, overall_precision + overall_recall
            )
