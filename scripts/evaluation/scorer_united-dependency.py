...
def read_file(path: str):
    ...
    with open(path, "r") as f:
        for line_no, line in enumerate(f):
            ...
            # End of sentence.
            if line == "":
                united_srl_id = document_id + "_" + sentence_id
                assert (
                    len(token_ids) == len(tokens) == len(lemmas) == len(roles)
                ), f"Line {line_no}: Lengths of token_ids, tokens, lemmas and roles do not match."
                assert (
                    united_srl_id not in data
                ), f"Line {line_no}: Duplicate UniteD-SRL ID {united_srl_id}."
            ...
            if line.startswith("#"):
                ...
                try:
                    ...
                    logging.error(
                        f'Line {line_no} is not a valid key-value pair (separated by " = ")'
                    )
                    exit(1)
                ...
                # Parse token.
                try:
                    token_id, token, lemma, frame, *line_roles = line.split("\t")
                except:
                    logging.error(f"Error in line {line_no}: {line}")
                    logging.error(
                        f'Line {line_no} is not a valid token line (separated by "\\t")'
                    )
                    exit(1)
                ...
                assert (
                    not token_ids or token_id == token_ids[-1] + 1
...
def evaluate(gold_data: dict, pred_data: dict) -> dict:
    ...
    for united_srl_id, gold_sentence in gold_data.items():
        if united_srl_id in pred_data:
            ...
            logging.warning(
                f"UniteD-SRL ID {united_srl_id} is not in the prediction data."
            )
        ...
        for token_id, gold_frame in gold_sentence["frames"].items():
            if token_id in pred_sentence["frames"]:
                ...
                logging.warning(
                    f"UniteD-SRL ID {united_srl_id}, token ID {token_id} is not in the prediction data."
                )
                pred_frame = "_"
...
def compute_micro_f1(data: dict) -> Dict[str, float]:
    ...
    precision = float(tp) / (float(tp) + float(fp)) if tp + fp > 0 else 0.0
    recall = float(tp) / (float(tp) + float(fn)) if tp + fn > 0 else 0.0
    f1 = (
...
def compute_macro_f1(data: dict) -> Dict[str, float]:
    ...
    precision /= len(result["precision"]) if len(result["precision"]) > 0 else 1.0
    recall /= len(result["recall"]) if len(result["recall"]) > 0 else 1.0
    f1 /= len(result["f1"]) if len(result["f1"]) > 0 else 1.0
...
def display_metrics(metrics: dict) -> None:
    ...
    print(
        "{:^15} | {:^15} | {:^15}".format("micro-P", "micro-R", "micro-F1").center(80)
    )
    ...
    print(
        "{:^15} | {:^15} | {:^15}".format("macro-P", "macro-R", "macro-F1").center(80)
    )
    ...
    print(
        "{:^15} | {:^15} | {:^15}".format("coarse-P", "coarse-R", "coarse-F1").center(
            80
        )
    )
    print("{:^15} | {:^15} | {:^15}".format("-" * 15, "-" * 15, "-" * 15).center(80))
