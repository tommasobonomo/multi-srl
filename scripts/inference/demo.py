    ...
    for i, tag in enumerate(bio_tags):
        if tag == "_":
            ...
            if start is not None:
                ...
                ), f"End of span is None but start of span is {start}."
                assert (
                    role is not None
                ), f"Role of span is None but start of span is {start}."
                if role != "V":
            ...
            # Continue span.
                ...
                assert (
                    start is not None
                ), f"Start of span is None but end of span is {end}."
                assert (
                    end is not None
                ), f"End of span is None but start of span is {start}."
                assert (
...
def group_spans(
    sentence: list, spans: dict, predicate_index: int, predicate_sense: str
) -> dict:
    # Group spans by role.
    # Example:
...
def srl(nlp: trankit.Pipeline, model: SrlParser, datamodule: SrlDataModule) -> list:
    ...
    def _srl(text: str, index: int) -> list:
        ...
        for batch_idx, batch in enumerate(datamodule.predict_dataloader()):
            batch_predictions = model.predict_step(
                batch, batch_idx, write_to_file=False
            )
        ...
        for sentence_id in sorted_sentence_ids:
            ...
            words = [w["text"] for w in doc["sentences"][sentence_id]["tokens"]]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vocabulary", type=str, required=True, dest="vocabulary_path")
    parser.add_argument(
        "--log",
        type=str,
        default="WARNING",
    ...
    nlp = trankit.Pipeline(lang="english", embedding="xlm-roberta-large")
